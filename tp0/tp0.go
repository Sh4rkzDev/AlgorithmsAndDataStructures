...
func Comparar(vector1 []int, vector2 []int) int {
	if len(vector2) > 0 && (len(vector1) == 0 || vector1[0] < vector2[0]) {
		return -1
	} else if len(vector1) > 0 && (len(vector2) == 0 || vector1[0] > vector2[0]) {
		return 1
	} else if len(vector1) == len(vector2) && len(vector1) == 0 {
		return 0
	}
...
func Seleccion(vector []int) {
	...
	for i := len(vector) - 1; i > 0; i-- {
		ind := Maximo(vector[:i+1])
		Swap(&vector[ind], &vector[i])
	}
}
